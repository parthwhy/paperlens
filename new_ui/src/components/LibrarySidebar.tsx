...
export function LibrarySidebar({ currentPaperId, onSelectPaper }: LibrarySidebarProps) {
  ...
  return (
    <aside className="w-72 h-full bg-surface-dim border-r-2 border-black flex flex-col shrink-0">
      <div className="p-4 border-b-2 border-black bg-surface">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {loading && papers.length === 0 ? (
          <div className="text-gray-500 text-sm animate-pulse">Loading history...</div>
        ) : papers.length === 0 ? (
          <div className="text-gray-500 text-sm italic">No papers ingested yet.</div>
        ) : (
          papers.map((paper) => (
            <button
              key={paper.paper_id}
              ...
              className={`w-full text-left p-3 rounded-lg border-2 transition-transform ${
                currentPaperId === paper.paper_id
                  ? 'bg-primary-light border-black brutal-shadow-sm -translate-y-[1px]'
                  : 'bg-surface border-transparent hover:border-black hover:-translate-y-[1px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]'
              }`}
            >
